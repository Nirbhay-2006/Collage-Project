    ...
    public class TeacherController : ControllerBase
    {
		...
		public async Task<IActionResult> DeleteCourse(int id)
		{
			...
			if (course.TeacherId != userId)
				return Forbid("Not your course");

			// ? DELETE FILES FROM SERVER
			if (course.CourseMedias != null)
			{
				foreach (var media in course.CourseMedias)
				{
					...
					Console.WriteLine(fullPath); // ?? debug path
				...
				_context.CourseMedias.RemoveRange(course.CourseMedias);
			}
		...
		public async Task<IActionResult> UploadMedia(int courseId, List<IFormFile> files)
		{
			...
			var uploadPath = Path.Combine("wwwroot/CourseVideos");
			...
			foreach (var file in files)
			{
				...
				var media = new CourseMedia
				{
					...
					FilePath = $"CourseVideos/{fileName}",
					FileType = file.ContentType.Contains("video") ? "video" : "image"
				};
		...
		{
			var media = await _context.CourseMedias.FindAsync(mediaId);

			if (media == null)
				return NotFound();

			var filePath = Path.Combine("wwwroot", media.FilePath);
